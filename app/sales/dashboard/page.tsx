...
export default function SalesDashboard() {
  ...
  if (loading) return <div className="flex h-screen bg-[#080808] text-white items-center justify-center"><p className="text-[10px] font-black uppercase tracking-widest text-zinc-400 animate-pulse">Loading...</p></div>;
  if (!currentStaff) return <div className="flex h-screen bg-[#080808] text-white items-center justify-center flex-col gap-6"><p className="text-[11px] font-black uppercase tracking-widest text-zinc-400">Account not linked to staff list</p><button onClick={handleLogout} className="text-[9px] font-black uppercase tracking-widest text-zinc-400 hover:text-white border border-white/10 px-6 py-3 rounded-xl">Log Out</button></div>;
      ...
      <aside className={`fixed inset-y-0 left-0 z-40 w-72 bg-black/95 backdrop-blur-xl border-r border-white/5 flex flex-col transform transition-transform duration-300 lg:relative lg:translate-x-0 ${sidebarOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0"}`}>
        <div className="p-6 lg:p-8 pb-8 flex items-start justify-between">
          <div>
            ...
            <p className="text-[9px] font-bold text-zinc-400 uppercase tracking-[0.3em] mt-1">Work Space</p>
            <p className="text-[9px] font-bold text-zinc-400 uppercase mt-1">{currentStaff.name} · {currentStaff.region}</p>
          </div>
        ...
        <div className="p-6 lg:p-8 border-t border-white/5">
          <button onClick={handleLogout} className="text-zinc-400 hover:text-rose-500 text-[10px] font-black uppercase tracking-widest flex items-center gap-2"><LogOut size={18}/> Logout</button>
        </div>
      </aside>
          ...
          <div className="flex items-center gap-3">
            {activeTab === "Customers" && !selectedCustomer && (
              <button onClick={() => { setIsEditing(false); setCustForm({ id: null, customer_name: "", phone: "", estate: "", court: "", house: "", prefs: "" }); setShowCustModal(true); }}
                ...
                <UserPlus size={14} fill="black"/> Add Customer
              </button>
            )}
            {activeTab === "Customers" && selectedCustomer && (
        ...
        <div className={`flex-1 overflow-hidden ${activeTab !== "Chat" ? "overflow-y-auto p-4 lg:p-12" : ""}`}>
            ...
            <div className="space-y-6 lg:space-y-10">
              <div className="grid grid-cols-2 gap-4 lg:gap-6">
                <div className="bg-white text-black p-6 lg:p-8 rounded-[2rem] shadow-xl">
                  ...
                  <p className="text-[9px] text-zinc-400 mt-2">Total contacts</p>
                </div>
                <div className="bg-white/[0.03] border border-white/5 p-6 lg:p-8 rounded-[2rem]">
                  <p className="text-[10px] font-black text-zinc-400 uppercase mb-1">My Orders</p>
                  <h3 className="text-3xl lg:text-4xl font-black italic">{allOrders.length}</h3>
                  <p className="text-[9px] text-zinc-400 mt-2">{todayOrders.length} today</p>
                </div>
              </div>
              {calls.length > 0 && (
                <div className="bg-white/[0.02] border border-white/5 p-6 lg:p-10 rounded-[3rem]">
                  <h4 className="text-xs font-black uppercase tracking-[0.3em] text-zinc-400 mb-6">Follow Up</h4>
                  <div className="space-y-3">
                    {calls.map(call => (
                      <div key={call.id} className="flex justify-between items-center p-4 lg:p-6 bg-white/[0.02] rounded-2xl border border-white/5">
                        <span className={`text-sm font-bold uppercase ${call.done?"line-through text-zinc-400":""}`}>{call.name}</span>
          ...
          {activeTab === "Customers" && !selectedCustomer && (
            <div className="space-y-4">
              <div className="flex items-center gap-3 bg-white/5 border border-white/5 rounded-2xl px-4 py-3">
                <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="text-zinc-400 flex-shrink-0"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
                <input value={customerSearch} onChange={e => setCustomerSearch(e.target.value)} placeholder="Search by name, phone or ID..." className="bg-transparent outline-none text-sm flex-1 text-white placeholder:text-zinc-400"/>
              ...
              {customers.length === 0 && (
                <div className="flex flex-col items-center justify-center py-24">
                  ...
                  <p className="text-[10px] font-black text-zinc-400 uppercase tracking-widest">No contacts yet</p>
                </div>
              )}
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 lg:gap-6">
                {filteredCustomers.map(c => (
                  <div key={c.id} className="bg-white/[0.02] border border-white/5 rounded-[2rem] p-6 lg:p-8">
                    ...
                    {c.address && <div className="mt-3 border-t border-white/5 pt-3"><p className="text-[9px] text-zinc-400 uppercase font-black mb-1">Location</p><p className="text-[10px] font-black">{c.address}</p></div>}
                    {c.preferences?.likes && <div className="mt-3 p-3 bg-white/5 rounded-2xl"><p className="text-[9px] text-zinc-400 uppercase font-black mb-1">Notes</p><p className="text-[10px] font-medium italic opacity-70">"{c.preferences.likes}"</p></div>}
                  </div>
                ))}
              </div>
            </div>
          ...
          {activeTab === "Customers" && selectedCustomer && (
            <div className="space-y-6">
              <button onClick={() => setSelectedCustomer(null)} className="flex items-center gap-2 text-[9px] font-black uppercase text-zinc-400 hover:text-white transition-all">
              ...
              <div className="bg-white/[0.02] border border-white/5 rounded-[2.5rem] p-6 lg:p-8">
                ...
                <p className="text-zinc-400 font-bold mt-1">{selectedCustomer.phone}</p>
                {selectedCustomer.address && <p className="text-[10px] text-zinc-400 mt-1">{selectedCustomer.address}</p>}
              </div>
              <div className="bg-white/[0.02] border border-white/5 p-6 lg:p-8 rounded-[3rem]">
                <p className="text-[9px] font-black text-zinc-400 uppercase tracking-widest mb-6">Order History</p>
                {customerOrders.length === 0 ? (
                  <div className="flex flex-col items-center py-12"><Package size={36} className="text-zinc-800 mb-3"/><p className="text-[10px] font-black text-zinc-400 uppercase">No orders yet — click Place Order</p></div>
                ) : (
                  <div className="space-y-4">
                    {customerOrders.map(order => (
                      <div key={order.id} className="bg-white/[0.02] border border-white/5 rounded-2xl p-5 lg:p-6">
                        <div className="flex justify-between items-start mb-3 flex-wrap gap-2">
                          <div><p className="text-[10px] font-black uppercase">{order.order_ref}</p><p className="text-[9px] text-zinc-400">{fmtDate(order.created_at)}</p></div>
                          <div className="flex items-center gap-3">
                        ...
                        <div className="space-y-1">{(order.items||[]).map((item: any, i: number) => (<p key={i} className="text-[9px] text-zinc-400">{item.name} × {item.quantity} — KSH {(item.subtotal||item.price*item.quantity).toLocaleString()}</p>))}</div>
                      </div>
                    ))}
                  </div>
                )}
                  ...
                  <>
                    <div>
                      <div className="flex items-center justify-between mb-4">
                        <p className="text-[9px] font-black text-zinc-400 uppercase tracking-widest">Today</p>
                      ...
                      {todayO.length === 0 ? (
                        <div className="bg-white/[0.02] border border-white/5 rounded-[2rem] p-10 flex flex-col items-center">
                          ...
                          <p className="text-[10px] font-black text-zinc-400 uppercase">No orders today yet</p>
                        </div>
                      ) : todayO.map(order => (
                        <div key={order.id} className="bg-white/[0.02] border border-white/5 rounded-2xl p-5 lg:p-6 mb-4">
                          <div className="flex justify-between items-start mb-3 flex-wrap gap-2">
                            <div><p className="text-sm font-black uppercase">{order.customers?.customer_name || "Customer"}</p><p className="text-[9px] text-zinc-400 mt-0.5">{order.order_ref} · {new Date(order.created_at).toLocaleTimeString("en-KE",{hour:"2-digit",minute:"2-digit"})}</p></div>
                            <div className="flex items-center gap-3">
                          ...
                          <div className="space-y-1 border-t border-white/5 pt-3">{(order.items||[]).map((item: any, i: number) => (<p key={i} className="text-[9px] text-zinc-400">{item.name} × {item.quantity} — KSH {(item.subtotal||item.price*item.quantity).toLocaleString()}</p>))}</div>
                        </div>
                      ))}
                    </div>
                    {olderO.length > 0 && (
                      <div>
                        <p className="text-[9px] font-black text-zinc-400 uppercase tracking-widest mb-4">Previous Orders</p>
                        {olderO.map(order => (
                          <div key={order.id} className="bg-white/[0.02] border border-white/5 rounded-2xl p-5 lg:p-6 mb-4">
                            <div className="flex justify-between items-start mb-3 flex-wrap gap-2">
                              <div><p className="text-[11px] font-black uppercase">{order.customers?.customer_name || "Customer"}</p><p className="text-[9px] text-zinc-400 mt-0.5">{order.order_ref} · {fmtDate(order.created_at)}</p></div>
                              <div className="flex items-center gap-3">
                                <p className="text-emerald-400 font-black">KSH {Number(order.total_amount).toLocaleString()}</p>
                                <button onClick={() => setShowInvoice({ ...order, customer: order.customers, staff: currentStaff })} className="flex items-center gap-1 px-3 py-2 bg-emerald-500/20 text-emerald-400 rounded-xl text-[9px] font-black uppercase"><Copy size={12}/> Invoice</button>
                              </div>
            ...
            <div className="h-full flex overflow-hidden bg-white/[0.02] border border-white/5 rounded-[3rem]">
              <div className="w-44 border-r border-white/5 flex flex-col flex-shrink-0">
                <button onClick={() => setDmWithAdmin(false)}
                  ...
                  <p className="text-[8px] text-zinc-400 pl-4">Everyone</p>
                </button>
                <button onClick={() => setDmWithAdmin(true)}
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-[8px] text-zinc-400">Private</p>
                    </div>
                  </div>
                </button>
              </div>
              <div className="flex-1 flex flex-col overflow-hidden">
                  ...
                  {dmWithAdmin ? (
                    <>
                      ...
                      <div>
                        ...
                        <p className="text-[8px] text-zinc-400">Direct message — only you and Admin see this</p>
                      </div>
                    </>
                  ) : (
                    <>
                      ...
                      <span className="text-[9px] text-zinc-400 ml-auto hidden lg:block">Everyone can see messages</span>
                    </>
                  )}
                </div>
                <div className="flex-1 overflow-y-auto p-4 lg:p-6 space-y-4">
                  {dmWithAdmin ? (
                    <>
                      {dmMessages.length === 0 && <p className="text-center text-[10px] font-black text-zinc-400 uppercase tracking-[0.5em] mt-12">No messages with Admin yet</p>}
                      {dmMessages.map((m, i) => {
                        const isMe = m.sender_id === currentStaff.id && !m.is_admin_sender;
                        return (
                          <div key={i} className={`flex flex-col ${isMe ? "items-end" : "items-start"}`}>
                            <p className="text-[8px] font-black text-zinc-400 uppercase mb-1 px-1">{isMe ? "You" : "Admin"}</p>
                    ...
                    <>
                      {messages.filter(m => m.receiver_id === "ALL").length === 0 && <p className="text-center text-[10px] font-black text-zinc-400 uppercase tracking-[0.5em] mt-12">No messages yet</p>}
                      {messages.filter(m => m.receiver_id === "ALL").map((m, i) => {
                        ...
                        return (
                          <div key={i} className={`flex flex-col ${isMe || isAdmin ? "items-end" : "items-start"}`}>
                            <p className="text-[8px] font-black text-zinc-400 uppercase mb-1 px-1">{senderLabel}</p>
      ...
      {showOrderCart && (
        <div className="fixed inset-0 bg-[#080808] z-50 flex flex-col overflow-hidden">
          <div className="flex items-center justify-between p-4 lg:p-8 border-b border-white/5 flex-shrink-0">
            <div><h2 className="text-xl font-black italic uppercase">New Order</h2><p className="text-[9px] text-zinc-400 mt-1">{selectedCustomer?.customer_name}</p></div>
            <button onClick={() => setShowOrderCart(false)}><X size={28}/></button>
          </div>
          <div className="flex flex-col lg:flex-row flex-1 overflow-hidden">
            <div className="flex-1 overflow-y-auto p-4 lg:p-8">
              <p className="text-[9px] font-black text-zinc-400 uppercase tracking-widest mb-4">Products</p>
              <div className="grid grid-cols-2 lg:grid-cols-3 gap-3 lg:gap-4">
                {products.map(p => (
                  <div key={p.id} className="bg-white/[0.03] border border-white/5 rounded-2xl overflow-hidden">
                    {p.image_url ? <img src={p.image_url} alt={p.name} className="w-full h-24 lg:h-28 object-cover"/> : <div className="w-full h-24 lg:h-28 bg-white/5 flex items-center justify-center"><Package size={24} className="text-zinc-400"/></div>}
            ...
            <div className="w-full lg:w-80 flex-shrink-0 border-t lg:border-t-0 lg:border-l border-white/5 flex flex-col max-h-64 lg:max-h-none">
              <div className="p-4 lg:p-6 flex-1 overflow-y-auto">
                <p className="text-[9px] font-black text-zinc-400 uppercase tracking-widest mb-4">Cart {cart.length>0&&`(${cart.length})`}</p>
                {cart.length === 0 ? <p className="text-[9px] text-zinc-400 mt-4 text-center">No items yet</p> : (
                  <div className="space-y-3">
                    {cart.map(item => (
                      <div key={item.id} className="bg-white/5 rounded-2xl p-3 lg:p-4">
                        <div className="flex justify-between items-start mb-2">
                          ...
                          <button onClick={() => removeFromCart(item.id)} className="text-zinc-400 hover:text-rose-400"><X size={14}/></button>
                        </div>
              ...
              <div className="p-4 lg:p-6 border-t border-white/5 flex-shrink-0">
                <div className="flex justify-between mb-4"><span className="text-[10px] font-black uppercase text-zinc-400">Total</span><span className="font-black text-emerald-400">KSH {cartTotal.toLocaleString()}</span></div>
        ...
        <div className="fixed inset-0 bg-black/95 backdrop-blur-xl z-50 flex items-center justify-center p-4">
          <div className="bg-[#0c0c0c] border border-white/10 w-full max-w-md p-8 lg:p-10 rounded-[3rem]">
            ...
            <div className="bg-white/5 rounded-2xl p-6 space-y-3 font-mono text-xs">
              ...
              <div className="border-t border-white/10 pt-3 text-zinc-400"><p>Sold by: {showInvoice.staff?.name} | {showInvoice.staff?.region}</p><p>BNS Systems</p></div>
            </div>
        ...
        <div className="fixed inset-0 bg-black/95 backdrop-blur-xl z-50 flex items-center justify-center p-4">
          <div className="bg-[#0c0c0c] border border-white/10 w-full max-w-lg p-8 lg:p-12 rounded-[3rem]">
            ...
            <form onSubmit={handleSaveCustomer} className="grid grid-cols-2 gap-4 lg:gap-6">
              <div className="col-span-2"><label className="text-[9px] font-black text-zinc-400 uppercase tracking-widest block mb-2">Full Name *</label>
              ...
              <div className="col-span-2"><label className="text-[9px] font-black text-zinc-400 uppercase tracking-widest block mb-2">Phone *</label>
              ...
              <div><label className="text-[9px] font-black text-zinc-400 uppercase tracking-widest block mb-2">Estate</label>
              ...
              <div className="flex gap-2">
                <div className="flex-1"><label className="text-[9px] font-black text-zinc-400 uppercase tracking-widest block mb-2">Court</label>
                ...
                <div className="flex-1"><label className="text-[9px] font-black text-zinc-400 uppercase tracking-widest block mb-2">House</label>
              ...
              <div className="col-span-2"><label className="text-[9px] font-black text-zinc-400 uppercase tracking-widest block mb-2">Notes</label>
