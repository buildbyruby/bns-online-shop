...
export default function OrderProcessorDashboard() {
    ...
    `}>
      <div className="p-6 lg:p-8 pb-8 lg:pb-12 flex items-center justify-between">
        <div>
          ...
          <p className="text-[9px] font-bold text-zinc-400 uppercase tracking-[0.3em] mt-1">Order Processor</p>
        </div>
        <button className="lg:hidden text-zinc-400 hover:text-white" onClick={() => setSidebarOpen(false)}>✕</button>
      </div>
      ...
      <div className="p-6 lg:p-8 border-t border-white/5">
        <button onClick={handleLogout} className="text-zinc-400 hover:text-rose-500 text-[10px] font-black uppercase tracking-widest flex items-center gap-2">
  ...
  if (loading) return (
    <div className="flex h-screen bg-[#080808] text-white items-center justify-center">
      <p className="text-[10px] font-black uppercase tracking-widest text-zinc-400 animate-pulse">Loading...</p>
    </div>
  );
    ...
    <div className="flex h-screen bg-[#080808] text-white font-sans overflow-hidden">
      ...
      <main className="flex-1 flex flex-col overflow-hidden min-w-0">
        <header className="h-16 lg:h-24 border-b border-white/5 flex items-center justify-between px-4 lg:px-12 flex-shrink-0">
          ...
          <button onClick={loadAll} className="flex items-center gap-2 text-[9px] font-black uppercase text-zinc-400 hover:text-white transition-colors">
        ...
        <div className="flex-1 overflow-y-auto p-4 lg:p-12 space-y-6 lg:space-y-8">
            ...
            <div className="space-y-6 lg:space-y-8">
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 lg:gap-6">
                {[
                  ...
                  <div key={i} className="bg-white/[0.03] border border-white/5 p-5 lg:p-8 rounded-2xl lg:rounded-[2rem]">
                    <p className="text-[9px] font-black text-zinc-400 uppercase tracking-widest mb-2 lg:mb-4">{m.label}</p>
                    <h3 className="text-xl lg:text-3xl font-black tracking-tighter italic">{m.val}</h3>
                    <p className="text-[9px] text-zinc-400 mt-1 lg:mt-2">{m.sub}</p>
                  </div>
                ))}
              </div>
              ...
              <div className="bg-white/[0.02] border border-white/5 p-6 lg:p-10 rounded-2xl lg:rounded-[3rem]">
                <div className="flex items-center justify-between mb-4 lg:mb-6">
                  <p className="text-[9px] font-black text-zinc-400 uppercase tracking-widest">Needs Action</p>
                ...
                {orders.filter(o => o.status === "pending").length === 0 ? (
                  <p className="text-[9px] text-zinc-400 text-center py-8">No pending orders 🎉</p>
                ) : (
                    ...
                    {orders.filter(o => o.status === "pending").slice(0, 8).map(order => (
                      <div key={order.id} className="flex flex-col sm:flex-row sm:items-center justify-between p-4 lg:p-5 bg-white/[0.02] rounded-2xl border border-white/5 gap-3">
                        <div>
                          ...
                          <p className="text-[9px] text-zinc-400">{order.customer?.customer_name || "Unknown"} · {order.salesperson?.name || "Unknown"}</p>
                          <p className="text-[9px] text-zinc-400">{fmtDate(order.created_at)} {fmtTime(order.created_at)}</p>
                        </div>
              ...
              {/* Customer summary */}
              <div className="bg-white/[0.02] border border-white/5 p-6 lg:p-10 rounded-2xl lg:rounded-[3rem]">
                <p className="text-[9px] font-black text-zinc-400 uppercase tracking-widest mb-4">Customers ({customers.length})</p>
                  ...
                  {customers.slice(0, 6).map(c => (
                    <div key={c.id} className="flex items-center gap-3 p-4 bg-white/[0.02] rounded-xl border border-white/5">
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="text-[9px] text-zinc-400 truncate">{c.phone}</p>
                      </div>
                    </div>
                  ))}
                </div>
                {customers.length > 6 && <p className="text-[9px] text-zinc-400 mt-3 text-center">+{customers.length - 6} more customers</p>}
              </div>
            </div>
          )}
          ...
          {activeTab === "Orders" && (
            <div className="space-y-4 lg:space-y-6">
              ...
              <p className="text-[9px] font-black text-zinc-400 uppercase tracking-widest">{filteredOrders.length} orders</p>

              {filteredOrders.length === 0 ? (
                <div className="text-center py-16"><p className="text-[10px] font-black text-zinc-400 uppercase">No orders found</p></div>
              ) : (
                    ...
                    <div key={order.id} className="bg-white/[0.02] border border-white/5 rounded-2xl lg:rounded-3xl p-5 lg:p-8">
                      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3 mb-4">
                        <div>
                          ...
                          <p className="text-[10px] text-zinc-400">👤 {order.customer?.customer_name || "Unknown"} · {order.customer?.phone || ""}</p>
                          <p className="text-[10px] text-zinc-400">🧑‍💼 {order.salesperson?.name || "Unknown"} · {order.salesperson?.region || ""}</p>
                          <p className="text-[9px] text-zinc-400 mt-1">{fmtDate(order.created_at)} at {fmtTime(order.created_at)}</p>
                        </div>
                        <div className="flex items-center gap-3">
                      ...
                      <div className="border-t border-white/5 pt-3 space-y-1">
                        {(order.items || []).map((item: any, i: number) => (
                          <p key={i} className="text-[9px] text-zinc-400">• {item.name} × {item.quantity} — KSH {(item.subtotal || item.price * item.quantity).toLocaleString()}</p>
                        ))}
                      </div>
                    </div>
                  ))}
            ...
            <div className="space-y-4 lg:space-y-6">
              <div className="grid grid-cols-3 gap-3 lg:gap-6">
                {[
                  ...
                  <div key={i} className="bg-white/[0.03] border border-white/5 p-5 lg:p-8 rounded-2xl lg:rounded-[2rem]">
                    <p className="text-[9px] font-black text-zinc-400 uppercase tracking-widest mb-2">{m.label}</p>
                  ...
                  <div key={s.id} onClick={() => setSelectedStaff(s)} className={`bg-white/[0.02] border rounded-2xl lg:rounded-3xl p-5 lg:p-8 cursor-pointer hover:border-white/20 transition-colors ${s.status === "suspended" ? "border-amber-500/20" : "border-white/5"}`}>
                    ...
                    {s.myOrders.slice(0,3).length > 0 && (
                      <div className="mt-4 pt-4 border-t border-white/5 space-y-2">
                        <p className="text-[8px] text-zinc-400 uppercase font-black">Recent</p>
                        {s.myOrders.slice(0,3).map((o: any) => (
                          <div key={o.id} className="flex justify-between items-center">
                            <p className="text-[9px] text-zinc-400">{fmtDate(o.created_at)}</p>
          ...
          {activeTab === "Analytics" && (
            <div className="space-y-6 lg:space-y-8">
              ...
              <div className="bg-white/[0.02] border border-white/5 p-6 lg:p-10 rounded-2xl lg:rounded-[3rem]">
                <p className="text-[9px] font-black text-zinc-400 uppercase tracking-widest mb-6 lg:mb-8">Revenue by Region</p>
                {revenueByRegion.length === 0 ? (
                  <p className="text-[9px] text-zinc-400 text-center py-8">No data yet</p>
                    ...
                    <div key={r.region} className="mb-5">
                      <div className="flex justify-between mb-2 flex-wrap gap-2">
                        <div className="flex items-center gap-2 lg:gap-3">
                          ...
                          <span className="text-[9px] text-zinc-400">{r.count} orders</span>
                        </div>
              ...
              <div className="bg-white/[0.02] border border-white/5 p-6 lg:p-10 rounded-2xl lg:rounded-[3rem]">
                <p className="text-[9px] font-black text-zinc-400 uppercase tracking-widest mb-6 lg:mb-8">Staff Leaderboard</p>
        ...
        <div className="fixed inset-0 bg-black/90 backdrop-blur-md z-50 flex items-center justify-center p-4" onClick={() => setSelectedStaff(null)}>
          <div className="bg-[#0c0c0c] border border-white/10 w-full max-w-lg p-8 rounded-[3rem] overflow-y-auto max-h-[90vh]" onClick={e => e.stopPropagation()}>
            <div className="flex justify-between items-center mb-8">
              ...
              <button onClick={() => setSelectedStaff(null)} className="text-zinc-400 hover:text-white text-xl font-black">✕</button>
            </div>
            <div className="space-y-2 mb-6">
              <div className="flex justify-between p-4 bg-white/5 rounded-2xl"><span className="text-[9px] font-black text-zinc-400 uppercase">Region</span><span className="text-[10px] font-black">{selectedStaff.region}</span></div>
              <div className="flex justify-between p-4 bg-white/5 rounded-2xl"><span className="text-[9px] font-black text-zinc-400 uppercase">Phone</span><span className="text-[10px] font-black">{selectedStaff.phone}</span></div>
              <div className="flex justify-between p-4 bg-white/5 rounded-2xl"><span className="text-[9px] font-black text-zinc-400 uppercase">Total Orders</span><span className="text-[10px] font-black">{selectedStaff.myOrders?.length || 0}</span></div>
              <div className="flex justify-between p-4 bg-white/5 rounded-2xl"><span className="text-[9px] font-black text-zinc-400 uppercase">Today Orders</span><span className="text-[10px] font-black text-blue-400">{selectedStaff.todayOrders || 0}</span></div>
              <div className="flex justify-between p-4 bg-white/5 rounded-2xl"><span className="text-[9px] font-black text-zinc-400 uppercase">Revenue</span><span className="text-[10px] font-black text-emerald-400">{fmt(selectedStaff.totalRevenue || 0)}</span></div>
            </div>
            {(selectedStaff.myOrders?.length || 0) > 0 && (
              <div>
                <p className="text-[9px] font-black text-zinc-400 uppercase tracking-widest mb-3">Order History</p>
                  ...
                  {selectedStaff.myOrders.map((o: any) => (
                    <div key={o.id} className="flex justify-between items-center p-3 bg-white/[0.02] rounded-xl border border-white/5">
                      <div>
                        ...
                        <p className="text-[9px] text-zinc-400">{fmtDate(o.created_at)} · {o.customer?.customer_name || "Unknown"}</p>
                      </div>
                      <div className="flex items-center gap-2">
                        <span className={`text-[8px] font-black uppercase px-2 py-0.5 rounded-full ${statusColor(o.status)}`}>{o.status}</span>
