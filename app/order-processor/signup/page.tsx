...
export default function OrderProcessorSignup() {
  ...
  const handleSignup = async (e: any) => {
    e.preventDefault();
    setLoading(true);
    const { error } = await supabase.auth.signUp({ email: formData.email, password: formData.password, options: { data: { full_name: formData.name, phone_number: formData.phone, role: "PROCESSOR" } } });
    if (!error) router.push("/order-processor/dashboard");
