...
export default function AdminLogin() {
  ...
  const handleLogin = async (e: any) => {
    ...
    if (signInData.user) { await supabase.from("admins").update({ last_login: new Date().toISOString() }).eq("id", signInData.user.id); }
    localStorage.setItem("userEmail", email); router.push("/admin/dashboard");
    setLoading(false);
  };
    ...
    <div className="flex min-h-screen items-center justify-center bg-black text-white px-5 py-10 sm:p-8">
      <div className="w-full max-w-sm space-y-8 sm:space-y-12">
        <div className="text-center space-y-2">
          ...
          <h1 className="text-2xl font-black tracking-widest uppercase italic">Admin Login</h1>
        </div>
        ...
        <div className="text-center space-y-4">
          ...
          <div className="flex flex-col gap-2">
            <Link href="/admin/signup" className="text-[9px] uppercase tracking-widest text-zinc-400 hover:text-white flex items-center justify-center gap-2">
              <UserPlus size={12}/> New Admin? Register here
            </Link>
